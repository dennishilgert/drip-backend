...
class SocketService implements ISocketService {
	...
	constructor(
		@inject(IdentityModule.DI_TYPES.IdentityService)
		identityService: IdentityModule.interfaces.IIdentityService
	) {
	...
	async registerClient(socket: Socket, uuid: string): Promise<void> {
		const identity: IdentityModule.types.IIdentity | null =
			await this.identityService.getIdentityByUuid(uuid)
		if (!identity) {
