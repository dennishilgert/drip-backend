...
import {
	BadIdentityDeletionDataError,
	BadIdentityUpdateDataError,
...
class IdentityService implements IIdentityService {
	...
	async validateIdentity(identityName: string): Promise<IIdentity> {
		const identity: IIdentity | null = await this.getIdentityByName(
			identityName
		)
		if (!identity) {
	...
	): Promise<IUpdateIdentityData> {
		const affectedRows: Array<number> = await this.identityRepo.updateByUuid(
			uuid,
			data
		)
		if (!affectedRows[0]) {
	...
	async deleteIdentity(uuid: string): Promise<boolean> {
		...
		if (affectedRows < 1) {
			throw new BadIdentityDeletionDataError(
				'Identity deletion affected 0 rows'
			)
		} else return true
	}
}
