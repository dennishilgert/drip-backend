...
class FileTransmissionRepo implements IFileTransmissionRepo {
  async create (createTransmission: ICreateFileTransmissionData): Promise<IFileTransmission> {
    return database.FileTransmission.create(createTransmission)
  }

  async getOneByCriteria (criteria: object, scopes?: string[] | false): Promise<IFileTransmission | null> {
  ...
  async getByUuid (uuid: string, scopes?: string[]): Promise<IFileTransmission | null> {
    return this.getOneByCriteria({ uuid }, scopes)
  }

  async getByRequestUuid (requestUuid: string, scopes?: string[]): Promise<IFileTransmission | null> {
    return this.getOneByCriteria({ requestUuid }, scopes)
  }

  async getManyByCriteria (criteria: object, scopes?: string[] | false): Promise<IFileTransmission[]> {
    const definedScopes = scopes || (scopes === false ? false : ['defaultScope'])
    const parameters: object = {
      where: criteria
    }
  ...
  async getByToUuid (toUuid: string, scopes?: string[]): Promise<IFileTransmission[]> {
