...
class MessageTransmissionRepo implements IMessageTransmissionRepo {
  ...
  async create (createTransmission: ICreateMessageTransmissionData): Promise<IMessageTransmission> {
    return database.MessageTransmission.create(createTransmission)
  }

  async getOneByCriteria (criteria: object, scopes?: string[] | false): Promise<IMessageTransmission | null> {
    const definedScopes = scopes || (scopes === false ? false : ['defaultScope'])
    const parameters: object = {
      where: criteria
    }
    if (definedScopes) {
      return database.MessageTransmission.scope(definedScopes).findOne(parameters)
    }
    return database.MessageTransmission.unscoped().findOne(parameters)
  }

  async getByUuid (uuid: string, scopes?: string[]): Promise<IMessageTransmission | null> {
    return this.getOneByCriteria({ uuid }, scopes)
  }

  async getManyByCriteria (criteria: object, scopes?: string[] | false): Promise<IMessageTransmission[]> {
    const definedScopes = scopes || (scopes === false ? false : ['defaultScope'])
    const parameters: object = {
      where: criteria
    }
    if (definedScopes) {
      return database.MessageTransmission.scope(definedScopes).findAll(parameters)
    }
    return database.MessageTransmission.unscoped().findAll(parameters)
  }

  async getByToUuid (toUuid: string, scopes?: string[]): Promise<IMessageTransmission[]> {
