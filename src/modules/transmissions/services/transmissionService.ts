...
import {
	IFileTransmissionRepo,
	IMessageTransmissionRepo,
...
class TransmissionService implements ITransmissionService {
	...
	public constructor(
		@inject(DI_TYPES.MessageTransmissionRepo)
		messageTransmissionRepo: IMessageTransmissionRepo,
		@inject(DI_TYPES.FileTransmissionRepo)
		fileTransmissionRepo: IFileTransmissionRepo,
		@inject(SocketModule.DI_TYPES.SocketService)
		socketService: SocketModule.interfaces.ISocketService
	) {
	...
	async cacheFile(
		...
		const uploadedFile: IUploadedFile = uploadedFiles.values().next()
	...
	sendRequest(toUuid: string, transmissionRequest: ITransmissionRequest): void {
		const socketClient: SocketModule.interfaces.ISocketClient | undefined =
			this.socketService.getClient(toUuid)
		if (!socketClient) {
			throw new IdentityModule.errors.IdentityNotConnectedError(
				'Target Identity is not connected'
			)
		}
			...
			.then(async (socketResponse: SocketModule.types.ISocketResponse) => {
				...
				switch (transmissionRequest.requestType) {
					case SocketModule.enums.RequestType.MESSAGE_TRANSMISSION: {
						...
						const affectedRows: number = await this.deleteMessageTransmission(
							transmissionRequest.requestUuid
						)
						if (affectedRows < 1)
							throw new TransmissionDeletionError(
								'Failed to delete message transmission'
							)
						break
					}
					case SocketModule.enums.RequestType.FILE_TRANSMISSION: {
						...
						const affectedRows: number = await this.deleteFileTransmission(
							transmissionRequest.requestUuid
						)
						if (affectedRows < 1)
							throw new TransmissionDeletionError(
								'Failed to delete file transmission'
							)
						break
					}
				}
	...
	async cleanFileTransmissions(identityUuid: string): Promise<void> {
		const fileTransmissions: IFileTransmission[] =
			await this.fileTransmissionRepo.getByToUuid(identityUuid)
	...
	async deleteFileTransmission(requestUuid: string): Promise<number> {
		const fileTransmission: IFileTransmission | null =
			await this.fileTransmissionRepo.getByRequestUuid(requestUuid)
