...
import { IFileTransmission, ICreateFileTransmissionData, ICreateMessageTransmissionData, IMessageTransmission, ITransmissionRequest, ITransmissionResponse, ITransformedFileTransmission, ITransformedTransmissionRequest, ITransformedMessageTransmission } from '../types'
...
class TransmissionService implements ITransmissionService {
  ...
  sendRequest (toUuid: string, transmissionRequest: ITransmissionRequest): void {
    ...
    const transformedTransmissionRequest: ITransformedTransmissionRequest = new TransmissionRequestTransformer(transmissionRequest).transform()

    socketClient.openRequest(transformedTransmissionRequest)
      .then(async (socketResponse: SocketModule.types.ISocketResponse) => {
        ...
        this.socketService.emitEvent(transmissionRequest.fromUuid, SocketModule.enums.SocketEvent.RESPONSE, JSON.stringify(transmissionResponse))
        ...
        switch (transmissionRequest.requestType) {
          case SocketModule.enums.RequestType.MESSAGE_TRANSMISSION: {
            if (transmissionResponse.accepted) {
              ...
              socketClient.emitEvent(SocketModule.enums.SocketEvent.MESSAGE_TRANSMISSION, JSON.stringify(transformedMessageTransmission))
              return
            }

            const affectedRows: number = await this.deleteMessageTransmission(transmissionRequest.requestUuid)
            if (affectedRows < 1) throw new TransmissionDeletionError('Failed to delete message transmission')
            break
          }
          case SocketModule.enums.RequestType.FILE_TRANSMISSION: {
            if (transmissionResponse.accepted) {
              ...
              socketClient.emitEvent(SocketModule.enums.SocketEvent.FILE_TRANSMISSION, JSON.stringify(transformedFileTransmission))
              return
            }
            const affectedRows: number = await this.deleteFileTransmission(transmissionRequest.requestUuid)
            if (affectedRows < 1) throw new TransmissionDeletionError('Failed to delete file transmission')
            break
          }
        }
      })
      .catch(async (error: Error) => {
        if (error instanceof SocketModule.errors.SocketRequestTimeoutError) {
          ...
          this.socketService.emitEvent(toUuid, SocketModule.enums.SocketEvent.REQUEST_RETRACTED, JSON.stringify(socketResponse))
          this.socketService.emitEvent(transmissionRequest.fromUuid, SocketModule.enums.SocketEvent.REQUEST_TIMEOUT, JSON.stringify(socketResponse))
  ...
  async cleanFileTransmissions (identityUuid: string): Promise<void> {
    ...
    fileTransmissions.forEach((fileTransmission: IFileTransmission) => {
      deleteFile(fileTransmission.filePath)
        ...
        .catch((error: Error) => {
          logger.error('File deletion error', { filePath: fileTransmission.filePath, error })
        })
    })
  }
