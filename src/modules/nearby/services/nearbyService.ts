...
class NearbyService implements INearbyService {
  ...
  public constructor (
    @inject(IdentityModule.DI_TYPES.IdentityRepo) identityRepo: IdentityModule.interfaces.IIdentityRepo
  ) {
  ...
  async getNearbyIp (excludeUuid: string, ip: string): Promise<IdentityModule.types.ITransformedIdentity[]> {
    ...
    nearbyIdentities.forEach((identity: IdentityModule.types.IIdentity) => {
      ...
      const transformedIdentity: IdentityModule.types.ITransformedIdentity = new IdentityApiTransformer(identity).transform()
   ...
   */
  async getNearbyGeolocation (excludeUuid: string, longitude: number, latitude: number): Promise<IdentityModule.types.ITransformedIdentity[]> {
