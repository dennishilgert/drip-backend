...
class NearbyService implements INearbyService {
	...
	async getNearbyIp(
		...
		const nearbyIdentities: IdentityModule.types.IIdentity[] =
			await this.identityRepo.getByIp(ip)
	...
	async getNearbyGeolocation(
		...
		const locationIdentities: IdentityModule.types.IIdentity[] =
			await this.identityRepo.getWithLocation()
		...
		locationIdentities.forEach((identity: IdentityModule.types.IIdentity) => {
			if (identity.uuid === excludeUuid) return
			const crow: number = this.calcCrow(
				longitude,
				latitude,
				identity.longitude,
				identity.latitude
