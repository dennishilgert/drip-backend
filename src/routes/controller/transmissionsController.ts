...
async function transmitMessage(
	...
	identityService
		...
		.then((toIdentity: IdentityModule.types.IIdentity) => {
			return transmissionService.cacheMessage(
				fromIdentity.uuid,
				toIdentity.uuid,
				message
			)
		...
		.then(
			(messageTransmission: TransmissionsModule.types.IMessageTransmission) => {
				...
				transmissionService.sendRequest(
					messageTransmission.toUuid,
					transmissionRequest
				)
...
async function transmitFile(req: Request, res: Response, next: NextFunction) {
	...
	identityService
		.validateIdentity(toName)
		.then((toIdentity: IdentityModule.types.IIdentity) => {
			return transmissionService.cacheFile(
				fromIdentity.uuid,
				toIdentity.uuid,
				req
			)
	...
	return transmissionService
		...
		.then((messageTransmission: IMessageTransmission | null) => {
			if (!messageTransmission) {
				throw new TransmissionsModule.errors.TransmissionNotFoundError(
					'Requested message transmission does not exist'
				)
			}
			transmissionService
...
async function retrieveFile(req: Request, res: Response, next: NextFunction) {
	...
	return transmissionService
		...
		.then(
			(
				fileTransmission: TransmissionsModule.types.IFileTransmission | null
			) => {
				if (!fileTransmission) {
					throw new TransmissionsModule.errors.TransmissionNotFoundError(
						'Requested file transmission does not exist'
					)
				}
				return res
					.status(200)
					...
					.download(fileTransmission.filePath, (error: Error) => {
						...
						transmissionService
							...
							.then((affectedRows: number) => {
								...
								logger.debug(
									`[${fileTransmission.fileName}] Deleted after download`
								)
							})
					})
			}
