...
const transmissionService: TransmissionsModule.interfaces.ITransmissionService = container.get(TransmissionsModule.DI_TYPES.TransmissionService)
...
async function transmitMessage (req: Request, res: Response, next: NextFunction) {
  ...
  identityService
    ...
    .then((messageTransmission: TransmissionsModule.types.IMessageTransmission) => {
...
async function retrieveMessage (req: Request, res: Response, next: NextFunction) {
  ...
  return transmissionService
    ...
    .then((messageTransmission: IMessageTransmission | null) => {
      ...
      transmissionService.deleteMessageTransmission(messageTransmission.requestUuid)
        .then((affectedRows: number) => {
          if (affectedRows < 1) logger.error('Failed to delete message transmission')
        })
      return res.status(200).type('text').send(messageTransmission.message)
    })
    .catch((error: Error) => {
      if (error instanceof TransmissionsModule.errors.TransmissionNotFoundError) {
        next(error)
      } else {
        logger.error('Download message transmission error', { toName: req.body.identity.name, error })
...
async function retrieveFile (req: Request, res: Response, next: NextFunction) {
  ...
  return transmissionService
    ...
    .then((fileTransmission: TransmissionsModule.types.IFileTransmission | null) => {
      ...
      return res.status(200).type(fileTransmission.fileMimeType).download(fileTransmission.filePath, (error: Error) => {
        if (error) throw error
        transmissionService.deleteFileTransmission(fileTransmission.requestUuid)
          .then((affectedRows: number) => {
            if (affectedRows < 1) {
    ...
    .catch((error: Error) => {
      if (error instanceof TransmissionsModule.errors.TransmissionNotFoundError) {
        ...
        logger.error('Download file transmission error', { toName: req.body.identity.name, error })
