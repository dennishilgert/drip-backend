...
const transmissionService: TransmissionsModule.interfaces.ITransmissionService = container.get(
...
async function retrieveMessage(req: Request, res: Response, next: NextFunction) {
  ...
  return transmissionService
    ...
    .then((messageTransmission: IMessageTransmission | null) => {
      ...
      transmissionService.deleteMessageTransmission(messageTransmission.requestUuid).then((affectedRows: number) => {
        if (affectedRows < 1) {
    ...
    .then((fileTransmission: TransmissionsModule.types.IFileTransmission | null) => {
      ...
      return res
        ...
        .download(fileTransmission.filePath, (error: Error) => {
          ...
          transmissionService.deleteFileTransmission(fileTransmission.requestUuid).then((affectedRows: number) => {
            if (affectedRows < 1) {
