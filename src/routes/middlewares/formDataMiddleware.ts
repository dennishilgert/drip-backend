...
function formDataParser(req: Request, res: Response, next: NextFunction): void {
  ...
  busboyParser.on('file', (fieldName: string, fileStream: internal.Readable, fileInfo: busboy.FileInfo) => {
    ...
    if (!isSupported(fileInfo.mimeType)) {
      fileStream.destroy()
      next(new ValidationError(`File mime type ${fileInfo.mimeType} is not supported`))
      return
    }
    ...
    fileStream.on('limit', () => {
      ...
      next(new ValidationError(`File exceeds file size limit of ${asNumber(process.env.FILE_MAX_SIZE)} bytes`))
    })
